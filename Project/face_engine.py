...
class FaceEngine:
    ...
    def _extract_face_encoding(self, image_array):
        try:
            DeepFace.extract_faces(
                image_array, enforce_detection=True
            )
    ...
    def compare_with_registered_face(self, registered_image_path, live_image_data, tolerance=0.45):
        ...
        live_image_array = self._decode_data_url(live_image_data)
        ...
        try:
            ...
            if "Face could not be detected" in str(e):
                return False, "No face detected in live image. Please keep your face clear."
            raise
